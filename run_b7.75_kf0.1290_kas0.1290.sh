...
#                                                             #
#    TODO: Add support for monitoring delay_sub_file.         #
#                                                             #
###############################################################
...
#PBS -A multirep
#PBS -N hmc_7.75_0.1290_0.1290_1
...
run_script="run_b${beta}_kf${kappa_f}_kas${kappa_as}.sh"
...
stream_name="hmc_${beta}_${kappa_f}_${kappa_as}_${label}"
...
then
    ...
    then
        ...
        cd ${PBS_O_WORKDIR}
        /usr/bin/rsh bc1p '/usr/local/pbs/bin/qsub -A multirep ${rootstore}/${run_script}'
    else
        ...
        echo "${PBS_O_WORKDIR}/${run_script}" >> ${delay_sub_file}
    fi
...
then
    cd ${PBS_O_WORKDIR}
    echo "Current working directory $PWD"
    echo "/usr/local/pbs/bin/qsub -A multirep ${rootstore}/${run_script}"
    /usr/bin/rsh bc1p "/usr/local/pbs/bin/qsub -A multirep ${rootstore}/${run_script}"
